 ...
 *
 * 초심자용 핵심 포인트:
 * - 여기서는 아직 CSV 파일에 아무것도 쓰지 않는다.
 * - 오직 SQL 문장을 읽어 "메모리 속 설계도(AST)"로 바꾸는 단계다.
 *
 * 읽는 예시 1:
 ...
 *
 * 초심자용 핵심 포인트:
 * - `SELECT *` 이면 select_all = 1 로 표시한다.
 * - `SELECT name, age` 처럼 별표가 아니면 column_names 배열에 담는다.
 * - WHERE가 있으면 parse_where_clause()가 최대 2개 조건까지 읽는다.
 *
 * 읽는 예시:
