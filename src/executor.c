 ...
 *
 * 두 가지 길:
 * - 인덱스가 있으면 후보 행 위치(row_offset)를 먼저 모은다.
 * - 인덱스가 없으면 CSV를 처음부터 끝까지 읽는다.
 *
 * @param config     실행 설정 구조체 포인터
 * @param statement  SELECT AST 포인터
 * @param error      오류 정보 저장 포인터
                          ...
                          ErrorInfo *error)
{
    ...
    if (used_index) {
        ...
        for (int offset_index = 0; offset_index < candidate_count; offset_index++) {
            /*
             * 인덱스를 썼더라도 WHERE를 한 번 더 검사한다.
             * age 인덱스만 써서 후보를 모았더라도,
             * WHERE age >= 20 AND id = 1 같은 다른 조건이 남아 있을 수 있다.
             */
            int value_count;
    ...
    char values[SQLPROC_MAX_COLUMNS][SQLPROC_MAX_VALUE_LEN];
    /* full scan 경로다.
     * 헤더 다음 줄부터 한 줄씩 읽으면서 조건을 검사한다.
     * 초심자 관점에서는 "가장 단순한 SELECT의 원형"으로 보면 된다.
     */
    while (fgets(line, sizeof(line), file) != NULL) {
        int value_count;
        memset(values, 0, sizeof(values));
