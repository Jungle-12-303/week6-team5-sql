...
이 프로젝트의 B+ 트리는 조회 성능을 더 살리는 쪽으로 구현되어 있습니다.
...
삽입은 루트에서 leaf까지 내려간 뒤 split을 위로 전파하고, 조회는 비교
연산자에 맞는 시작 leaf를 찾은 뒤 필요한 leaf chain만 앞으로 따라갑니다.
...
6. 인덱스가 있으면 B+ 트리에서 시작 leaf를 찾고 필요한 leaf만 읽어 row
   offset 후보를 모읍니다.
7. 여러 WHERE 조건 중 인덱스에 쓰지 않은 조건은 실행기에서 다시 검사합니다.
